...
│
└── references/
    ├── workflows.md         - 16个排查流程的检查项清单
    ├── commands-mapping.md  - 检查项 → 具体命令的完整映射表（结构化表格）
    └── analysis-guide.md    - 详细的解读要点和风险判断标准
...
5. 快速导航：SKILL.md 中提供清晰的流程选择和 references 链接
  ...
  1. 用户选择"系统信息排查"
     ↓
  2. 在 workflows.md 中查看→ 系统信息排查 的检查项列表
     ↓
  3. 在 commands-mapping.md 中查找→ 用户信息分析 的命令
     ↓
  4. 执行命令获取输出
     ↓
  5. 在 analysis-guide.md 中查看→ UID=0用户 的解读要点
     ↓
  6. 给出专业分析报告
